"""Push-to-talk daemon: X hotkey loop, audio capture, whisper transcription,
and the transcript sinks.

All heavy imports (numpy, sounddevice, faster-whisper, Xlib) live here so
`wspr` subcommands never pay for them; wspr.py imports this module only for
the bare-daemon invocation.
"""
...
def run(cfg: dict) -> None:
    """Grab the configured hotkeys and run the record/transcribe loop until Ctrl-C."""
